...
There are two ways to begin. Editors can upload the `.eml` forwarded by the Support Notes coordinator and click **Parse and extract** — the app parses the file, calls Claude Haiku to fill the slot schema, and presents the populated form. Alternatively, clicking **Start from scratch** opens the same form with all fields blank, pre-set to the selected language and the current year and month, for cases where the editor prefers to copy-paste content manually without an `.eml` source.
