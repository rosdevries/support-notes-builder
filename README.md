...
**Email parsing and AI extraction (`.eml`):** The `.eml` file is parsed to isolate the editor-facing preview HTML and any supplementary notes embedded in the email body. Both blocks are passed to Claude Haiku, which maps the content into a structured schema covering the header title and strapline, subscribe link, two editorial sections (each with a quote, speaker name and title, editorial paragraph, and left- and right-column article links), the upcoming-webinar schedule, the latest-release download links, the webinar series link, an archive footnote, and the CAN-SPAM-compliant footer. Metadata fields — product name, language, year, and month — are always derived from the email subject line and are never left to the model. If the Anthropic API key is not configured, or if the AI call fails, the parser falls back to a heuristic extraction that fills the unambiguous fields and leaves the rest blank for manual editing.

**PowerPoint template parsing (`.pptx`):** The Support Notes content template is a PowerPoint file whose second slide contains a structured table — one cell per editorial section, speaker block, resource link, webinar callout, and latest-release component. The PPTX parser reads this table directly without calling the AI: cell positions are fixed by the template design, so field extraction is deterministic and instant. Hyperlinks embedded in the table cells are resolved from the slide's relationship dictionary, so all article and release-component URLs are captured automatically. Speaker headshot images embedded on subsequent slides are extracted and presented as thumbnail candidates in the UI — clicking **Use** on any thumbnail composites and uploads it to SFMC without a separate upload step. Because the PPTX has no equivalent of the email subject line, the product name, year, and month fields are left blank for the editor to fill in, and the language defaults to whichever language is selected in the Language picker before parsing.

**Streamlit editing UI:** After parsing, every extracted field is presented as an editable widget — rich Quill editors for HTML fields, plain text inputs for labels and URLs, and a drag-to-reorder interface for article links. The subscribe button label and URL are independently editable; leaving either blank uses the language-appropriate default. The footer field renders a live dark-background preview so editors can check link contrast before publishing. A reset button on each field restores the language-appropriate default. The UI supports five languages: English (`en`), Japanese (`ja`), Korean (`ko`), Simplified Chinese (`zh-CN`), and Traditional Chinese (`zh-TW`). Progress is auto-saved to a local draft store keyed by product, language, year, and month, so reopening the same email picks up where the editor left off.
...
There are three ways to begin. Editors can upload the `.eml` forwarded by the Support Notes coordinator and click **Parse and extract** — the app parses the file, calls Claude Haiku to fill the slot schema, and presents the populated form. Editors can instead upload the `.pptx` content template completed by the product team: the PPTX parser fills the same form fields directly from the table structure without an AI call, and any headshot images embedded in the file appear as one-click upload options in the speaker photo column. Alternatively, clicking **Start from scratch** opens the same form with all fields blank, pre-set to the selected language and the current year and month, for cases where the editor prefers to copy-paste content manually.

Once the form is open, the editor reviews and corrects any fields the AI got wrong, uploads or confirms speaker headshots, and adjusts article links or webinar dates as needed. When satisfied, they click **Create email in SFMC** to publish the asset to Content Builder. The resulting asset ID and a link to SFMC are shown on screen.
...
**Running tests:** `python -m pytest tests/ -v`. The test suite covers the `.eml` parser, the `.pptx` parser, the slot renderer, and bootstrap-template logic offline without SFMC or Anthropic access.
